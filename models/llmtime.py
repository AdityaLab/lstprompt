...
def generate_predictions(
    ...
    max_concurrent=10,
    prompt_method="LSTP",all_time_steps=-1,breath_steps=-1,dataname="NA",
    **kwargs
):
    """
    ...
    completions_list = []
    # print('input strs in llmtime.py')
    # print(input_strs)
    # print("#####################################")
    complete = lambda x: completion_fn(input_str=x, steps=steps*STEP_MULTIPLIER, settings=settings, num_samples=num_samples, temp=temp,prompt_method=prompt_method,all_time_steps=all_time_steps,breath_steps=breath_steps,dataname=dataname)
...
def get_llmtime_predictions_data(train, test, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, prompt_method="LSTP",all_time_steps=-1,breath_steps=-1,dataname="NA",**kwargs):
    """
    ...
    # print("serialized input_strs in llmtime.py")
    # print(input_strs)
    ...
    if num_samples > 0:
        preds, completions_list, input_strs = generate_predictions(completion_fn, input_strs, steps, settings, scalers,
                                                                    ...
                                                                    parallel=parallel,prompt_method=prompt_method,all_time_steps=all_time_steps,breath_steps=breath_steps,dataname=dataname, **kwargs)
        ...
        samples = samples if len(samples) > 1 else samples[0]
        medians = medians if len(medians) > 1 else medians[0]
        #means = means if len(means) > 1 else means[0]
